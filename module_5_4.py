class House: #1.Создаем класс
    ...
    houses_history = [] # 1. Создаем атрибут класса  *** (Задача "История строительства")

    def __new__(cls, *args, **kwargs):
        instance = super(House, cls).__new__(cls)  # 2. Создаем новый объект  *** (Задача "История строительства")
        if args:
...
#          return self.number_of_floors != other.number_of_floors
    ...
    def __del__(self):
        print(f"{self.name} снесён, но он останется в истории") # 3. переопределяем метод __del__(self) в котором будет выводиться строка:"<название> снесён, но он останется в истории" *** (Задача "История строительства")
...
# 4. Создаём несколько объектов класса House и проверьте работу методов __del__ и __new__, а также значение атрибута houses_history. *** (Задача "История строительства")
h1 = House('ЖК Эльбрус', 10)
...
print(House.houses_history) # 5. Проверяем работу атрибута 'houses_history' *** (Задача "История строительства")